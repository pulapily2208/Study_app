...
import com.example.study_app.ui.Deadline.Models.Icon;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS icon (" +
                "id_icon INTEGER PRIMARY KEY AUTOINCREMENT," +
                "src_icon TEXT NOT NULL" +
                ");");

        // Thêm dữ liệu mẫu
        db.execSQL("INSERT INTO icon (src_icon) VALUES ('brain');");
        db.execSQL("INSERT INTO icon (src_icon) VALUES ('star');");
        db.execSQL("INSERT INTO icon (src_icon) VALUES ('bell');");

        runSqlFromRaw(db, R.raw.study_app);  // các bảng khác
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS icon"); // Corrected table name
...
//    SUBJECT
    ...
    // --- DEADLINE & ICON METHODS ---

    /**
     * Lấy tất cả các icon từ database.
     */
    public List<Icon> getAllIcons() {
        List<Icon> iconList = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        try (Cursor cursor = db.rawQuery("SELECT * FROM icon ORDER BY icon_id ASC", null)) {
            if (cursor.moveToFirst()) {
                do {
                    Icon icon = new Icon();
                    icon.setId_icon(cursor.getInt(cursor.getColumnIndexOrThrow("icon_id")));
                    icon.setName_icon(cursor.getString(cursor.getColumnIndexOrThrow("src_icon")));
                    icon.setSrc_icon(cursor.getString(cursor.getColumnIndexOrThrow("src_icon"))); // Lưu ý: Schema của bạn dùng `icon_path`
                    iconList.add(icon);
                } while (cursor.moveToNext());
            }
        } catch (Exception e) {
            Log.e("DatabaseHelper", "Error while getting all icons", e);
        }
        return iconList;
    }
    ...
    public ArrayList<Deadline> getDeadlinesByMaHp(String maHp) {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT d.*, i.icon_path FROM Deadlines d " +
                       "LEFT JOIN Icons i ON d.icon_id = i.icon_id " +
                       "JOIN Weeks w ON d.week_id = w.week_id WHERE w.ma_hp = ?";
        ...
        try (Cursor cursor = db.rawQuery(query, new String[]{maHp})) {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    deadline.setTieuDe(cursor.getString(cursor.getColumnIndexOrThrow("title")));
                    deadline.setNoiDung(cursor.getString(cursor.getColumnIndexOrThrow("note")));
                    deadline.setNgayBatDau(parseDateTime(cursor.getString(cursor.getColumnIndexOrThrow("start_datetime"))));
                    deadline.setNgayKetThuc(parseDateTime(cursor.getString(cursor.getColumnIndexOrThrow("end_datetime"))));
    ...
    public long addDeadline(Deadline deadline, String maHp) {
        ...
        values.put("icon_id", deadline.getIcon()); // Lưu DB ID của icon
        values.put("repeat_type", deadline.getRepeat() != null ? deadline.getRepeat() : "once");
